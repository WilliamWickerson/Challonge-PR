angular.module('listings').controller('ListingsController', ['$scope', '$rootScope', '$location', '$stateParams', '$state', 'Listings', 
  function($scope, $rootScope, $location, $stateParams, $state, Listings){
    $scope.myFilter = function (item) {
      if(!($scope.query) || (item.tag.toLowerCase().indexOf($scope.query.toLowerCase()) != -1))
	return item;
    };
	...
	name: $scope.name,
      ...
      Listings.addBracket(link)
	.then(function(response) {
	  $state.go('listings.list', { successMessage: 'Bracket succesfully added!' });
        }, function(error) {
	  $scope.error = 'Unable to edit listing!\n' + error;
        });
    };
	...
	password: $scope.password
      ...
      Listings.check(info)
	.then(function(response) {
	  $rootScope.log = $scope.username.toLowerCase();
	  $state.go('listings.list', { successMessage: 'Bracket succesfully added!' });
        }, function(error) {
	  $scope.error = 'Username/Password Combination not found';
        });
    };
